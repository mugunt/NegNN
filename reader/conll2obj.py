...
class _DataSetInventory(dict):
    ...
    def get(self, name):
        if name not in self: raise ValueError('unknown dataset' + name)
...
class Sentence(list):
    ...
    def pretty(self, annotation_id=None):
        ...
        for i in range(len(self)):
            ...
            event_starts = Frequencies()
            event_ends = Frequencies()
            ...
            for identifier in ids:
                ...
                in_event[identifier] = \
                    Token._indices(in_event[identifier], current, following,
                                    identifier, Annotation.event_accessor,
            ...
            for i, char in enumerate(self[i].word):
                ...
                for j in range(event_starts[i]): string += _event_markers[0]
                for j in range(cue_starts[i]): string += _cue_markers[0]
                string += char
                for j in range(cue_ends[i]): string += _cue_markers[1]
                for j in range(event_ends[i]): string += _event_markers[1]
...
class Token(object):
    ...
    def to_conll(self):
        fields = [self.sentence.chapter, str(self.sentence.number),
    ...
    def to_ptb(self):
	return self.syntax.replace('*', '(%s %s)' % (ptb_escape_brackets(self.pos),
                                                     ptb_escape_brackets(self.lemma)))
...
class Annotation(object):
    ...
    def fields(self):
        return [Annotation._out(field) for field in
                (self.cue, self.scope, self.event)]
