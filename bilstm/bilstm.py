...
from NegNN.utils.tools_tf import *
    ...
    def __init__(self,
        ...
        with tf.device("/cpu:0"):
            if external:
                ...
                self._weights = {
                'w_emb' : random_uniform([voc_size, emb_dim],'w_emb'),
                'c_emb' : random_uniform([3,emb_dim],'c_emb')
                }
                if tags:
        ...
        pred = [tf.nn.tanh(tf.matmul(item, self._weights['out_w']) + self._biases['out_b']) for item in pred]
        outputs = tf.squeeze(tf.pack(pred))
