...
parser.add_argument('-pre_training',action="store_true",help="File containing the pickled version")
parser.add_argument('-pkl',help="File containing the pickled version")
...
#         _,_,test_set,_ = cPickle.load(f)
#         # _,val_set,_,_ = cPickle.load(f)
#     _,_,_,_,cues,_ = test_set[0]
#     # _,_,_,_,cues,_ = val_set
#     return cues

# def load_dict_ofs(train_lex,):
#     with open(pkl_file,'rb') as f:
#         train_set,_,_,_dict = cPickle.load(f)
...
if not pre_training:
    ...
    _, _, voc, dic_inv = unpickle_data(args.pkl)
...
sents = [SentenceAnalysis(sent,gold,_sys,cue_sent) for (sent,gold,_sys),cue_sent in izip(test,test_cues)]
