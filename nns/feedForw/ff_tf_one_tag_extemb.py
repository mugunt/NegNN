...
from NegNN.utils.tools_tf import shuffle, minibatch, contextwin_lr
...
if __name__=="__main__":
    ...
    folder = os.path.join("/Users/ffancellu/git/is13/log",params['logf'])
    ...
    sess.run(tag_emb.assign(t_syn0))
    ...
    try:
        for e in xrange(params['nepochs']):
            if args.t:
                shuffle([train_lex,train_y,train_cue,train_tags], params['seed'])
            if args.u:
                shuffle([train_lex,train_y,train_cue,train_tags_uni], params['seed'])
            ...
            for i in xrange(nsentences):
                ...
                if args.t:
                    ctags = contextwin_lr(train_tags[i],params['win_left'], params['win_right'],'word',ntags)
                if args.u:
                    ctags = contextwin_lr(train_tags_uni[i],params['win_left'], params['win_right'],'word',ntags)
                labels = map(lambda x: numpy.asarray([1,0]).astype('int32') if x ==0 else numpy.asarray([0,1]).astype('int32'),train_y[i])
                sess.run(train_step, feed_dict={x: cwords, c: ccues, t: ctags, y_: labels,lr: params['clr']})    
            ...
            for i in xrange(len(valid_lex)):
                ...
                if args.t:
                    ctags_dev = contextwin_lr(valid_tags[i],params['win_left'], params['win_right'],'word',ntags)
                if args.u:
                    ctags_dev = contextwin_lr(valid_tags_uni[i],params['win_left'], params['win_right'],'word',ntags)
                labels_dev = map(lambda x: numpy.asarray([1,0]).astype('int32') if x == 0 else numpy.asarray([0,1]).astype('int32'),valid_y[i])
                acc = sess.run(accuracy, feed_dict={x: cwords_dev, c: ccues_dev, t: ctags_dev, y_: labels_dev})
                dev_acc += acc
