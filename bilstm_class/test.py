...
allow_soft_placement = str2bool(model_flags['allow_soft_placement'])
log_device_placement = str2bool(model_flags['log_device_placement'])
...
def unpickle_data():
       with open(os.path.join(FLAGS.checkpoint_dir,'train_dev.pkl'),'wb') as f:
...
if not pre_training:
    ...
    test_lex, test_tags, test_tags_uni, test_cue, test_scope, test_y = int_processor.load_test(test_files, voc, scope_detection, event_detection, FLAGS.test_lang)
else:
    ...
    test_lex, test_tags, test_tags_uni, test_cue, test_scope, test_y = test_set
...
if POS_emb in [1,2]:
    ...
    else: tag_voc_size = len(voc['t2idxs']) if POS_emb == 1 else len(voc['tuni2idxs'])
else: tag_voc_size = 0
